...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.from("events").insert({ ...body, organizer_id: user.id }).select().single();
...
export async function PATCH(request: Request) {
  ...
  const { data, error } = await supabase.from("events").update(updates).eq("id", id).eq("organizer_id", user.id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ event: data });
}
